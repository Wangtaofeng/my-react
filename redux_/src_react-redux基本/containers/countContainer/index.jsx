...
// 这里通过props与组件连接（不需要手动）, 并与store连接
...
// connect方法需要传递两个参数, 均为函数, 一个返回状态, 一个返回操作状态的方法，并且是一个高阶函数，第二次调用传入组件
// 下面是两个函数
// 第一个函数,即connect的第一个参数，返回store中的状态, 参数为store中的state,该返回值在组件中的props中
// 每次store中的该数据改变时，都会重新执行该方法，即组件的props会更新
function mapStateToProps (state) {
  // 需要返回一个对象
  return { count: state }
}
//  第二个函数,即connect的第二个参数，返回操作store状态的方法, 参数为dispatch, 可以用来通知store去操作状态
